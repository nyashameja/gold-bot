...
final class EncryptionTest extends TestCase
{
    ...
    {
        self::assertTrue($this->encryption->isEncrypted($this->encryption->encrypt('x')));
        self::assertFalse($this->encryption->isEncrypted('plain text'));
    }

    public function test_the_version_prefix_separates_two_applications(): void
    {
        $key = Encryption::generateKey();
        $goldBot = new Encryption($key, 'gb1:');
        $other = new Encryption($key, 'nd1:');

        // Same key, different version tag. The tag is what identifies a stored
        // ciphertext, so two applications sharing the kernel do not silently
        // share a ciphertext namespace.
        self::assertFalse($other->isEncrypted($goldBot->encrypt('x')));
        self::assertTrue($goldBot->isEncrypted($goldBot->encrypt('x')));
    }
}
